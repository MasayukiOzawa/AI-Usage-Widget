...
Claude Code のローカル資格情報からアクセストークンを読み、使用量エンドポイントへ読み取りリクエストだけを送ります。アクセストークンの有効期限が15分以内になると、保存済みリフレッシュトークンをClaude Code CLIへ渡して`claude auth login`による更新を試みます。トークンは子プロセス専用の環境変数で渡し、コマンドラインやログへ出力しません。モデルへの生成リクエストは行いません。

カードの「詳細」には、アクセストークン期限、自動更新を開始する時刻、リフレッシュトークン期限を表示します。Claude Codeの資格情報にリフレッシュトークン期限が保存されていない場合は「期限情報なし（自動更新可能）」と表示し、期限を推測しません。

#### 認証期限切れ時の挙動

更新に失敗してもアクセストークンが有効な間は、そのトークンで使用枠の取得を継続します。リフレッシュトークンがない、必要なスコープがない、リフレッシュトークンが期限切れまたは無効で、アクセストークンも期限切れになった場合は、使用枠の前回値を残したまま状態が「未接続」になります。Claude Desktopの利用だけでは、Claude Code用の資格情報が更新されるとは限りません。

`claude auth status` はログイン状態を確認するコマンドであり、期限切れトークンの更新や接続復旧を保証しません。自動更新できず「未接続」になった場合は、PowerShellなどで次のコマンドを実行し、ブラウザー認証を完了してください。
